...
class Post extends Component {
    ...
    handleThumbUp = (e) => {
        ...
        this.props.addVote(postId, params)
    }
    ...
    render() {
        ...
        return (<Fragment>
            <Grid container="container" spacing={16}>
                <Grid item="item" sm={1} xs={1}/>
                <Grid item={true} sm={10} xs={10}>
                    <CardForm handleThumbUp={this.handleThumbUp} handleThumbDown={this.handleThumbDown} handleEdit={this.handleModalOpen} handleDeletePost={this.handleDeletePost} thumbUpColor={thumbUpColor} thumbDownColor={thumbDownColor} title={title} timeString={timeString} body={body} author={author} voteScore={voteScore} buttonDisabled={buttonDisabled} isPost={true}/>
                </Grid>
                <Grid item="item" sm={1} xs={1}/>
            </Grid>
            <Grid container="container" spacing={16}>
            ...
            <Grid container="container" spacing={16}>
                ...
                <Grid item="item" sm={10} xs={10}>
                    {
                        commentList
                            ? commentList.map(comment => <CommentList key={comment.id} comment={comment}/>)
                            : null
                    }
                </Grid>
                <Grid item="item" sm={1} xs={1}/>
            ...
            <Grid container="container" spacing={16}>
                ...
                <Grid item="item" sm={10} xs={10}>
                    {
                        addComment
                            ? <CommentForm formName="Add Comment" handleCommentForm={this.handleAddComment} handleSubmit={this.submitComment} isAdd={true}/>
                            : null
                    }
                </Grid>
                <Grid item="item" sm={1} xs={1}/>
            </Grid>
            <PostModal open={showModal} handleClose={this.handleModalClose} style={classes.dialog} submitForm={this.submitForm} author={author} title={title} body={body}/>
        </Fragment>);
    }
}
...
export default compose(withStyles(DefaultStyles), connect(mapStateToProps, postAction),)(Post);
